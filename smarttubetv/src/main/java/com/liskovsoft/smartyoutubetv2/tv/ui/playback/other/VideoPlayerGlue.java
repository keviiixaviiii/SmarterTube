...
import com.liskovsoft.smartyoutubetv2.tv.ui.playback.actions.BackgroundAudioAction;
...
public class VideoPlayerGlue extends MaxControlsVideoPlayerGlue<PlayerAdapter> implements OnActionLongClickedListener {
    ...
    public VideoPlayerGlue(
        ...
        putAction(new BackgroundAudioAction(context));
    }
    ...
    protected void onCreatePrimaryActions(ArrayObjectAdapter adapter) {
        ...
        if (mPlayerTweaksData.isPlayerButtonEnabled(PlayerTweaksData.PLAYER_BUTTON_BACKGROUND_AUDIO)) {
            adapter.add(mActions.get(R.id.action_background_audio));
        }
    }
    ...
    public void setCompactControls(boolean compact) {
        ...
        // Re-sync just that icon to the real player state; otherwise, while already playing (no
        // state event to correct it), the icon stays stale — most visibly after rotating into
        // landscape, where the full control set is rebuilt mid-playback (see
